...
select customer_id,product_name from sales s
join menu m
on s.order_date in
(select min(order_date)  from sales s
group by customer_id)  and s.product_id = m.product_id
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
-- 4. What is the most purchased item on the menu and how many times was it purchased by all customers?
select  customer_id,product_name,count(s.product_id) as most_purchase from menu m
join sales s
...
with cte as (
select *,DENSE_RANK() over(partition by customer_id order by total_time desc) as rnk from(
select customer_id,product_id,count(product_id)  as total_time from sales
group by customer_id,product_id) as t
)
select customer_id,product_name,total_time from cte
join menu m
...
on order_date > join_date and s.customer_id=m.customer_id
join menu me 
on s.product_id = me.product_id) as t
where rnk = 1
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
with cte as
(select s.*,m.join_date,me.product_name,me.price from sales s
inner join members m 
on order_date < join_date and s.customer_id=m.customer_id
join menu me
on s.product_id = me.product_id)

select cte.customer_id,count(distinct cte.product_id) as total_item,sum(price) as total_spent from cte
group by cte.customer_id
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
-- 9.  If each $1 spent equates to 10 points and sushi has a 2x points multiplier - how many points would each customer have?
with cte as (
select *, case when product_name in ('curry','ramen') then price*10 else price*20 end as points from(
select customer_id,product_name,sum(price) as price from sales s
join menu m on s.product_id = m.product_id 
group by customer_id,product_name)as t)

select customer_id, sum(points) total_points from cte
group by customer_id
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
with cte as (
select s.customer_id,case when order_date <= join_date then price*10
when order_date > join_date then price * 20 end as points
from sales s
...
select avg(n_days) from (
select *,DATEDIFF(day,start_date,end_date) as n_days from customer_nodes where end_date != '99991231') as t

--What is the median, 80th and 95th percentile for this same reallocation days metric for each region?
select distinct t.region_id,r.region_name,PERCENTILE_CONT(0.5) within group (order by diff) over(partition by t.region_id) as median, 
PERCENTILE_CONT(0.8) within group (order by diff) over(partition by t.region_id) as median,
PERCENTILE_CONT(0.95) within group (order by diff) over(partition by t.region_id) as median from(
select *,DATEDIFF(day,start_date,end_date) as diff from customer_nodes where end_date != '99991231') as t
...
-- What is the percentage of customers who increase their closing balance by more than 5%?
  select * from customer_transactions
  -- What is the unique count and total amount for each transaction type?
  select txn_type,count(*), sum(txn_amount)as total_amt from customer_transactions
  group by txn_type
...
SELECT 
  ...
  select avg(txn_count), avg(avg_amount) from cte
...
SELECT 
  ...
  select months ,count(*) from cte
  where deposit_count >1 and (purchase_count = 1 or withdrawal_count = 1)
  group by months

 -- What is the closing balance for each customer at the end of the month?
 select *,ROW_NUMBER() over(partition by customer_id order by txn_date) as rn from customer_transactions